...
window.login = async () => {
  ...
  try {
    ...
    if (statusEl) {
      statusEl.innerText = `Login failed: ${e?.code || 'unknown error'}`;
...
window.toggleTips = () => {
  ...
  // Temporarily lock the app to prevent artificial inputs from triggering a save
  const wasReady = isAppReady;
  isAppReady = false;
  document.querySelectorAll('.ghost-input').forEach((el) => el.dispatchEvent(new Event('input')));
  isAppReady = wasReady;
};
